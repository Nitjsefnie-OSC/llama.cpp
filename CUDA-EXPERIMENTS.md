...
045 independent attribution review passes: cuda-host-phases-capture2-attribution-review-v1.json SHA256 ff99c695890f2f3046c101bd958c58f271be67def560881634c0ade7d03d547c. Independent recomputation confirms the corrected root/logger/residual partition and optimistic envelopes: repeated getters0.1083%/0.1269%, input waits0.1623%/0.1658%, complete checkpoint-copy erasure3.3543%/0.8918%. These are observed diagnostic costs, not removable-work predictions; none supports >=2% at both prompt lengths. Checkpoint allocation/value initialization remains unmeasured. Source-reference correction to the preceding entry: data_tgt.resize is at common/common.cpp:2306, not2307; the earlier entry is preserved under the append-only rule. No new source or service change.


Final bounded CUDA feasibility is NO-GO: cuda-pq2-next-feasibility-v1.json SHA256 671043c807b80d8706cf73ce1345d0bc027d37f529729569994d7525667b9f49, cost artifact SHA256692e5b86dc836b7162a9f5035eebee9986c3bf20389f1ac8cf08fa62b7c2b7f0. The retained fused PQ2 MMVQ runs40 calls/~5.702ms per measured output step at both prompt lengths, K5120,17408 rows, grid4352x1x1/block32x4x1,37 registers and no stack/local/shared storage. Reaching2% total output gain would require at least9.644%/10.363% faster execution of that category using an optimistic recorded-kernel-only denominator; service overhead is outside that denominator. This is a target requirement, not evidence the kernel can improve that much.

A distinct eligibility hypothesis permits original F32 activation/output overlap after the existing separate Q8 snapshot. Same-stream ordering could protect the activation read, but exact graph/leaf/consumer/alias eligibility is not proved. Replacing24 observed unfused pairs with existing fused behavior models only about0.43% benefit, or0.47% even erasing every intervening recorded chain gap. Moderate implementation effort and alias-correctness risk are unsupported by that cost estimate, so no source prototype, build or GPU/service experiment follows. Prior permutation sharing, fixed-K unrolling, no-unroll, indexing, payload/lookup, Q8 layout and staged-prefill variants are explicitly excluded as repeats. The cached profiler warnings and incomplete-collection limitation remain; no bandwidth-saturation or occupancy claim.

Offline cost analysis exits0 in4.332s. A guessed helper-path read failed (logs/cuda-next-decode-discovery-v1.py absent); its PowerShell nonterminating error was masked by a later successful Python command and is explicitly treated as failed evidence. The final analysis instead uses saved selection JSON plus complete bounded SQLite assertions. Five exploratory console displays exceeded output budgets without changing saved inputs; no failed native/GPU run occurred.

Following the user's question about whether more iteration is worthwhile, this optimization pass closes at four retained throughput wins. Recent source, diagnostic and cost checks provide no evidenced next candidate meeting the existing >=2% service threshold; this is diminishing-return judgment, not proof that all future optimization is impossible. Accepted037 remains served. The default-inert host tracing, strict validator, corrected launcher and all successful/failed experiment records are retained in the fork. No further experiment is queued for this pass.
